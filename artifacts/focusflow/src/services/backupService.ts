...
import type { AppSettings, BlockPreset, AllowedAppPreset, RecurringBlockSchedule, PendingPresets, Task } from '@/data/types';
...
export async function restoreFromJson(
  ...
  // ── Settings ─────────────────────────────────────────────────────────────
  //
  // Imports MUST NOT silently overwrite the user's live enforcement settings.
  // Anything that controls active blocking (block list, daily allowances,
  // deterrents, content guards, profile) is staged into `pendingPresets`
  // and surfaced as an Apply / Dismiss banner on the relevant screen.
  //
  // What we DO merge directly (low-risk, no enforcement impact):
  //   • UI prefs:           darkMode, beginnerMode, tipsCard*, *Seen flags
  //   • Defaults:           defaultDuration, defaultReminderOffsets,
  //                         pomodoroEnabled/Duration/Break,
  //                         notificationsEnabled, keepFocusActiveUntilTaskEnd,
  //                         overlayWallpaper, overlayQuotes
  //   • Library additions:  blockPresets, allowedAppPresets,
  //                         recurringBlockSchedules — appended (de-duped by id)
  //                         so the user gets imported templates without losing
  //                         their own. They are NOT auto-enabled.
  try {
    const importedAt = new Date().toISOString();
    const sourceName = `Backup (${env.exportedAtHuman})`;
    const e = env.settings;

    // Build the pendingPresets bundle (only set categories that have data).
    const pending: PendingPresets = { ...(cb.currentSettings.pendingPresets ?? {}) };

    if (e.standaloneBlockPackages?.length) {
      pending.blockApps = {
        packages: e.standaloneBlockPackages,
        sourceName,
        importedAt,
      };
    }
    if (e.dailyAllowanceEntries?.length) {
      pending.dailyAllowance = {
        entries: e.dailyAllowanceEntries,
        sourceName,
        importedAt,
      };
    }
    if (e.aversionDimmerEnabled || e.aversionVibrateEnabled || e.aversionSoundEnabled) {
      pending.deterrents = {
        aversionDimmerEnabled: e.aversionDimmerEnabled,
        aversionVibrateEnabled: e.aversionVibrateEnabled,
        aversionSoundEnabled: e.aversionSoundEnabled,
        sourceName,
        importedAt,
      };
    }
    if (
      e.systemGuardEnabled ||
      e.blockInstallActionsEnabled ||
      e.blockYoutubeShortsEnabled ||
      e.blockInstagramReelsEnabled ||
      (e.blockedWords?.length ?? 0) > 0
    ) {
      pending.enforcement = {
        systemGuardEnabled: e.systemGuardEnabled,
        blockInstallActionsEnabled: e.blockInstallActionsEnabled,
        blockYoutubeShortsEnabled: e.blockYoutubeShortsEnabled,
        blockInstagramReelsEnabled: e.blockInstagramReelsEnabled,
        blockedWords: e.blockedWords,
        sourceName,
        importedAt,
      };
    }
    if (e.userProfile && Object.keys(e.userProfile).length > 0) {
      pending.profile = {
    ...
    // De-dupe library additions by id — keep current entries, append unseen.
    const mergeById = <T extends { id: string }>(current: T[], incoming: T[]): T[] => {
      const seen = new Set(current.map((x) => x.id));
      const extra = incoming.filter((x) => x && x.id && !seen.has(x.id));
      return [...current, ...extra];
    };
    const blockPresets: BlockPreset[] = mergeById(
      cb.currentSettings.blockPresets ?? [],
      e.blockPresets ?? [],
    );
    const allowedAppPresets: AllowedAppPreset[] = mergeById(
      cb.currentSettings.allowedAppPresets ?? [],
      e.allowedAppPresets ?? [],
    );
    const recurringBlockSchedules: RecurringBlockSchedule[] = mergeById(
      cb.currentSettings.recurringBlockSchedules ?? [],
      (e.recurringBlockSchedules ?? []).map((s) => ({ ...s, enabled: false })),
    );

    const merged: AppSettings = {
      ...cb.currentSettings,
      // Low-risk UI / default fields that are safe to overwrite.
      darkMode: e.darkMode ?? cb.currentSettings.darkMode,
      defaultDuration: e.defaultDuration ?? cb.currentSettings.defaultDuration,
      defaultReminderOffsets: e.defaultReminderOffsets ?? cb.currentSettings.defaultReminderOffsets,
      pomodoroEnabled: e.pomodoroEnabled ?? cb.currentSettings.pomodoroEnabled,
      pomodoroDuration: e.pomodoroDuration ?? cb.currentSettings.pomodoroDuration,
      pomodoroBreak: e.pomodoroBreak ?? cb.currentSettings.pomodoroBreak,
      notificationsEnabled: e.notificationsEnabled ?? cb.currentSettings.notificationsEnabled,
      keepFocusActiveUntilTaskEnd: e.keepFocusActiveUntilTaskEnd ?? cb.currentSettings.keepFocusActiveUntilTaskEnd,
      overlayWallpaper: e.overlayWallpaper ?? cb.currentSettings.overlayWallpaper,
      overlayQuotes: e.overlayQuotes ?? cb.currentSettings.overlayQuotes,
      beginnerMode: e.beginnerMode ?? cb.currentSettings.beginnerMode,
      // Library additions — never overwrite, only append.
      blockPresets,
      allowedAppPresets,
      recurringBlockSchedules,
      // Stage everything dangerous behind the apply/dismiss banner.
      pendingPresets: pending,
    };
  ...
  // ── Tasks ─────────────────────────────────────────────────────────────────
  // All tasks coming through .focusflow restore are flagged as `imported` so
  // they show up in stats history without firing live alarms or claiming the
  // "current task" slot. See notificationService.scheduleTaskReminders and
  ...
  for (const t of env.tasks) {
    ...
    try {
      const taskWithFlag: Task = {
        ...(t as Task),
        imported: true,
        importedAt,
      };
      await cb.addTask(taskWithFlag);
