...
const GUIDE: GuideSection[] = [
  ...
  },
  {
    ...
    color: COLORS.purple,
    steps: [
      { heading: 'Block Batches', body: 'Create one or more time-window batches — pick apps and the hours/days they should be blocked (e.g. no Instagram 9–17 Mon–Fri). Each batch runs independently. Set once, runs forever.' },
...
export default function HowToUseScreen() {
  ...
  /**
   * Mark the how-to-use guide as seen and route home. Setting `howToUseSeen`
   * unlocks the ConceptTourModal gate so it can fire on first home-tab paint.
   * Used by every "exit" path in onboarding mode (CTA, Skip link, header
   * back button, hardware back).
   */
  const finishOnboarding = async () => {
    try {
      if (!state.settings.howToUseSeen) {
        await updateSettings({ ...state.settings, howToUseSeen: true });
      }
    } catch { /* non-fatal — flag will retry next visit */ }
    router.replace('/');
  };
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 40 + insets.bottom }]}>
        ...
        <View style={[styles.noticeBanner, { backgroundColor: `${COLORS.primary}12`, borderColor: `${COLORS.primary}28` }]}>
          ...
          <Text style={[styles.noticeText, { color: theme.text }]}>
            ...
            {' '}tasks, blocked apps, block batches, and presets from the main app.
          </Text>
        </View>
