...
export default function UserProfileScreen() {
  ...
  // ── "Switching from another blocker?" shortcut ───────────────────────────
  // Lifted out of the onboarding (permissions) screen so it lives where it
  // makes sense: the profile screen, after permissions are granted. Imported
  // packages are saved as a NEW BlockPreset — never dumped into the always-on
  // block list — so the user explicitly chooses when to start enforcement.
  const [importOtherVisible, setImportOtherVisible] = useState(false);
  const handleImportFromOtherBlocker = async (packages: string[]) => {
    ...
    Alert.alert(
      'Saved as a preset',
      `${result.added} app${result.added !== 1 ? 's' : ''} saved as "${result.preset.name}".\n\nOpen Standalone Block from the side menu and pick this preset whenever you're ready to start blocking.`,
    );
  };
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {!isEditMode && (
            <>
              ...
              {/* Switching from another blocker? — relocated from onboarding
                  so users land on it AFTER permissions are granted, where the
                  imported list can actually be enforced. */}
              <TouchableOpacity
                style={styles.switchBanner}
                onPress={() => setImportOtherVisible(true)}
                activeOpacity={0.85}
              >
                <View style={styles.switchBannerIconWrap}>
                  <Ionicons name="swap-horizontal" size={22} color="#fff" />
                </View>
                <View style={styles.switchBannerTextWrap}>
                  <Text style={styles.switchBannerTitle}>Switching from another blocker?</Text>
                  <Text style={styles.switchBannerBody}>
                    Bring your block list across — Stay Focused, AppBlock, Lock Me Out, and more.
                  </Text>
                </View>
      ...
      <ImportFromOtherAppModal
        visible={importOtherVisible}
        onClose={() => setImportOtherVisible(false)}
        onImport={handleImportFromOtherBlocker}
      />
    </SafeAreaView>
  );
}
